const submit = document.querySelector('#send-all-messages');

submit.addEventListener("click", (event) => { // Execute when id #send-all-messages is clicked
  var forms = document.querySelectorAll(".total-forms"); // Get all the forms into an array
  for (var i=0 ; i<forms.length ; i++){ // Forms.length makes sure that the loop continues until the total amount of forms is met
    document.querySelector("#click-message-form"+i).click(); // In the send_messages_card.html.erb the index of the form has been added to the submit button. With this code all submit buttons of the existing forms are clicked
  }
